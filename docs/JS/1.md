...
let Andy = new Star('Andy',34)
let Red = new Star('Red',50)
```
我们创建的每个实例对象都会单独的开辟一块内存空间存储相应的数据,像`name`,`age`这些简单数据类型还好,但像`function`这种复杂数据类型,如果内存为每个对象的复杂数据类型都单独开辟一块空间的话,会非常浪费内存资源资源.**如下图**:</br></br>
![prototype](../images/prototype.svg)</br>
...
```
我们可以在浏览器控制台`dir(Array.prototype)`打印数组的公有方法
## 对象原型 __proto__
...
`__proto__`对象原型的意义就在于为对象的查找机制提供一个方向，或者说一条路线，但是它是一个非标准属性，因此实际开发中，不可以使用这个属性，它只是内部指向原型对象`prototype`**如图:**</br></br>
...
```
## 原型链
每一个实例对象又有一个`__proto__`属性，指向的构造函数的原型对象，构造函数的原型对象也是一个对象，也有`__proto__`属性，这样一层一层往上找就形成了原型链。如图<br><br>
![prototypeLink](../images/prototypeLink.svg)<br>>
**原型链和成员的查找机制**<br>
...
## new 运算符的作用
new 运算符除了和执行普通函数执行形成一个全新的执行上下文-->初始化`arguments`,形参赋值-->初始化作用域链之外,还额外的进行了一些操作<br>
- 默认创建一个对象,这个对象就是当前类的实例
- 声明 this 指向, 让其指向这个新创建的实例
- 构造函数中不论是否有 return 都会把新创建的实例返回(如果手动返回一个引用数据类型数据则会把实例覆盖掉)
...
let sanmao = _new(Dog, '美旺');
sanmao.bark(); //=>"汪汪"
sanmao.sayName(); //=>"my name is 美旺"
console.log(sanmao instanceof Dog); //=>true
```
_new函数的手动实现
```js
function _new(Func, ...arguments) {
    // 1.创建一个空实例对象, 将 __proto__指向 Func.prototype
    let obj = Object.create(Func.prototype)
    // 2.将 this 指向新创建的实例对象,并且赋值(执行函数)
    let result = Func.call(obj, ...arguments)
    // 3.如果返回值为引用数据类型就覆盖掉
...
let arr = [1, 2, 3];/* 相当与let = new Array(1,2,3) */
arr.sum();
